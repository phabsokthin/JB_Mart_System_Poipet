...
function CreateExpenseType({ onClose, fetchData, onSuccess, dataUpdate }: { onClose: () => void, fetchData: () => void, onSuccess: (message: string) => void, dataUpdate: dataUpdate | any }) {
    ...
    const handleSubmit = async (event: any) => {
        ...
        try {
            ...
            if (!dataUpdate) {
                ...
                const response = await axios.post(`${url}expenseType`, data);
                ...
                const response = await axios.put(`${url}expenseType/${dataUpdate.expenseTypeId}`, data);
                if (response.data.msg) {
            ...
            >
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4 space-y-2">
                        <label className="block font-bold text-gray-700 font-NotoSansKhmer ">ប្រភេទចំណាយ: *</label>
                        <input
                            type="text"
                            value={names}
                            onChange={(e) => setNames(e.target.value)}
                            required
                            className="input_text" placeholder="ប្រភេទចំណាយ"
                        />
                    </div>
                    <div className="mb-4 space-y-2">
