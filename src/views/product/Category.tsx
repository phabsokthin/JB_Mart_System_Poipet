...
function Category() {
    ...
    const [dataToDelete, setdataToDelete] = useState<{ categoryId: number; cnames: string } | null>(null);
    ...
    const filtereData = category.filter((data) =>
        data.cnames.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    // Delete handling
    const handleDeleteClick = (cnames: { categoryId: number; cnames: string }) => {
        setdataToDelete(cnames);
        console.log(cnames)
    ...
    const handleDelete = async () => {
        if (dataToDelete !== null) {
            try {
                await axios.delete(`${url}category/${dataToDelete.categoryId}`);
                setCategory(category.filter(data => data.categoryId !== dataToDelete.categoryId));
                    ...
                    <div>
                        ...
                        <div className="flex items-center justify-between gap-2 mb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <p className="text-xl font-bold font-NotoSansKhmer">តារាងបញ្ជីប្រភេទទំនេញ</p>
                            </div>
                            <button
                                ...
                                {currentData.length > 0 ? (
                                    currentData.map((item, index) => (
                                        <tr key={item.categoryId || index}>
                                            ...
                                            <td className="px-4 py-2 w-[12.5%]">{item.cnames}</td>
                                            ...
                                            <td className="px-4 py-2 w-[12.5%] space-x-2">
                                                <button
                                                    onClick={() => handleDeleteClick({ categoryId: item.categoryId, cnames: item.cnames })}
                        ...
                        <AnimatePresence>
                            {isDeleteModalOpen && (
                                <DeleteAction
                                    ...
                                    displayName={dataToDelete?.cnames || "មិនមានឈ្មោះ"}
                                />
                            )}
                        </AnimatePresence>
